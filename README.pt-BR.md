...
A segurança faz parte da interface: arquivos existentes no alvo são preservados por padrão, mutações de API não são repetidas automaticamente após falhas de transporte, credenciais não são exibidas e qualquer execução real precisa ser solicitada explicitamente.
...
O Makefile detecta Windows através de `OS=Windows_NT`: usa `python` por padrão no Windows e `python3` em ambientes POSIX. O executável pode ser sobrescrito, por exemplo:
...
O Makefile resolve esses valores usando o mesmo parser de `.env` da CLI Python. Assim, normalmente você configura o alvo uma vez em vez de repetir `TARGET=` e `REPO=` em todos os comandos.

### Validar antes do setup
...
Com o `.env` configurado, o fluxo normal fica:
...
- `discover` inspeciona o alvo local configurado e recomenda um setup seguro.
- `init-dry` mostra os arquivos que seriam instalados sem criar o diretório-alvo.
- `setup` instala os arquivos locais ausentes e em seguida executa a fase configurada do GitHub em dry-run. Ele **não** aplica alterações remotas.

Após revisar a saída, a execução completa real continua explícita:
...
O caminho local e o repositório remoto são normalmente lidos do `.env`. Dry-run continua sendo o padrão para operações remotas mutáveis. Use `LIVE=1` somente depois de revisar a simulação.
...
| `make init-dry` | Dry-run | Mostra quais arquivos locais seriam instalados sem criar o diretório-alvo. |
| `make init` | Escrita local | Instala arquivos ausentes preservando os já existentes. |
| `make setup` | Instalação local + dry-run remoto | Instala arquivos ausentes e depois simula a fase configurada da API. |
| `make plan` | Dry-run | Simula a fase completa configurada da API do GitHub. |
| `make apply` | Dry-run | Executa a fase configurada sem escrita. Use `LIVE=1` para aplicar. |
...
Se o número do Project ainda não estiver salvo no `.env`:

```bash
make project-sync PROJECT_NUMBER=1
```

Qualquer alvo/repositório persistente pode ser sobrescrito para apenas uma execução:
